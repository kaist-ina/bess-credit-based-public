...
PacketPool *PacketPool::default_huge_pools_[RTE_MAX_NUMA_NODES];
...
void PacketPool::CreateDefaultPools(size_t capacity) {
  ...
  for (int sid = 0; sid < NumNumaNodes(); sid++) {
    if (FLAGS_m == 0) {
      ...
      LOG(INFO) << "Creating DpdkPacketPool (Huge) for " << (capacity/4)
                << " packets on node " << sid;
      default_huge_pools_[sid] = new DpdkPacketPool(capacity/4, sid, true);
    } else {
...
PacketPool::PacketPool(size_t capacity, int socket_id, bool huge_pkt) {
  ...
  pool_ = rte_mempool_create_empty(name_.c_str(), capacity, huge_pkt ? SNBUF_HUGE_DATA : sizeof(Packet),
...
void PacketPool::PostPopulate(bool huge) {
  PoolPrivate priv = {
      .dpdk_priv = {.mbuf_data_room_size = static_cast<uint16_t>(SNBUF_HEADROOM + (huge ? SNBUF_HUGE_DATA : SNBUF_DATA)),
...
DpdkPacketPool::DpdkPacketPool(size_t capacity, int socket_id, bool huge_pkt)
    : PacketPool(capacity, socket_id, huge_pkt) {
...
Packet *PacketPool::from_paddr(phys_addr_t paddr) {
  for (int i = 0; i < RTE_MAX_NUMA_NODES; i++) {
    ...
    if (!default_huge_pools_[i]) {
      continue;
    }
    pool = default_huge_pools_[i]->pool();
