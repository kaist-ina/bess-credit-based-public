...
class PacketPool {
 public:
  ...
  static PacketPool *GetDefaultHugePool(int node) { return default_huge_pools_[node]; }

  static void CreateDefaultPools(size_t capacity = kDefaultCapacity);

  // socket_id == -1 means "I don't care".
  PacketPool(size_t capacity = kDefaultCapacity, int socket_id = -1, bool huge_pkt = false);
  virtual ~PacketPool();
 ...
 protected:
  ...
  void PostPopulate(bool huge = false);
 ...
 private:
  ...
  static PacketPool *default_huge_pools_[RTE_MAX_NUMA_NODES];
...
class DpdkPacketPool : public PacketPool {
 public:
  DpdkPacketPool(size_t capacity = kDefaultCapacity, int socket_id = -1, bool huge_pkt = false);
